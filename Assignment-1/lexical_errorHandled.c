...
#define STRING_LITERAL_TOKEN 263
#define CHAR_CONST_TOKEN 264
#define L_PARENTHESIS_TOKEN 265
#define R_PARENTHESIS_TOKEN 266
#define L_CURLY_BRACE_TOKEN 267
#define R_CURLY_BRACE_TOKEN 268
#define L_SQUARE_BRACKET_TOKEN 269
#define R_SQUARE_BRACKET_TOKEN 270
#define SEMICOLON_TOKEN 271
#define COMMA_TOKEN 272
#define HEADER_FILE_TOKEN 273
#define MACRO_TOKEN 274
#define ADDRESS_OF_TOKEN 275
#define POINTER_TOKEN 276
#define COMMENT_TOKEN 278
#define EOF_TOKEN 279
#define UNKNOWN_TOKEN 280
#define RELATIONAL_OP_TOKEN 281
#define FLOAT_CONST_TOKEN 282
...
void append_token_char() {
    if (yyleng < MAX_TOKEN_LENGTH - 1) {
        ...
        fprintf(stderr, "Error: Token length exceeded at line %d\n", current_line);
    }
}
...
int yylex() {
    ...
    if (isalpha(current_char) || current_char == '_') {
        ...
        if (yyleng >= MAX_TOKEN_LENGTH) {
            fprintf(stderr, "Error: Identifier length exceeded at line %d\n", current_line);
            return ERROR_TOKEN;
        }
    ...
    if (is_num(current_char)) {
        int has_dot = 0;
        while (is_num(current_char) || (current_char == '.' && !has_dot)) {
            if (current_char == '.') {
                has_dot = 1;
            }
            append_token_char();
        ...
        if (has_dot) {
            return FLOAT_CONST_TOKEN;
        } else if (yyleng >= MAX_TOKEN_LENGTH) {
            fprintf(stderr, "Error: Numeric constant length exceeded at line %d\n", current_line);
            return ERROR_TOKEN;
        }
        return INT_CONST_TOKEN;
    }
    ...
    if (!isprint(current_char) || current_char == '$') {
        fprintf(stderr, "Error: Illegal character '%c' encountered at line %d\n", current_char, current_line);
        fetch_char();
        return ERROR_TOKEN;
    }
...
int main(int argc, char *argv[]) {
    ...
    while (!feof(yyin)) {
        ...
        if (token == ERROR_TOKEN) {
            fprintf(stderr, "Lexical error detected at line %d: %s\n", current_line, yytext);
        } else {
            printf("%-50s %-10d\n", yytext, token);
        }
    }
